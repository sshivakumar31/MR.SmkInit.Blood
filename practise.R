...
# Add error bars
bp <- bp + geom_errorbar(data = summary_data, aes(xmin = mean - se, xmax = mean + 
se), width = 0.2)

# Add P-value text
bp <- bp + geom_text(data = summary_data, aes(x = 0, label = paste("P =", pval)), 
y = 1, vjust = -1.5, color = "red")

# Save the plot as PDF
ggsave("boxplot1.pdf", plot = bp)
